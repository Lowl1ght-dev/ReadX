/** GLSL из https://codepen.io/cameronknight/pen/ogxWmBP (Interactive Liquid Gradient, Three.js) */
...
export const liquidFragmentShader = `
  ...
  float grain(vec2 uv, float time) {
    /* Было: uv * resolution * 0.5 — слишком мелкий «телевизионный» шум.
       Крупная, медленная фактура + мягкая амплитуда в коде материала. */
    vec2 g = uv * 72.0 + vec2(time * 0.06, time * 0.045);
  ...
  void main() {
    ...
    float raw = touchTex.b;
    float touch = pow(clamp(raw, 0.0, 1.0), 0.82) * 0.24;
    uv.x += vx * touch;
    uv.y += vy * touch;

    vec2 center = vec2(0.5);
    float dist = length(uv - center);
    float tRipple = pow(clamp(raw, 0.0, 1.0), 0.85);
    float ripple = sin(dist * 20.0 - uTime * 3.0) * 0.01 * tRipple;
    float wave = sin(dist * 15.0 - uTime * 2.0) * 0.007 * tRipple;
    uv += vec2(ripple + wave);
    ...
    color.r += sin(timeShift) * 0.008;
    color.g += cos(timeShift * 1.4) * 0.008;
    color.b += sin(timeShift * 1.2) * 0.008;
